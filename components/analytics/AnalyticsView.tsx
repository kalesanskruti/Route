...
export function AnalyticsView() {
  ...
  const attendanceTrendsData = data?.attendanceTrendsData || [];
  const routeEfficiencyData = data?.routeEfficiencyData || [];
  const driverPerformanceData = data?.driverPerformanceData || [];
  const studentDeptData = data?.studentDeptData || [];
  const occupancyData = data?.occupancyData || [];
  const maintenanceCostData = data?.maintenanceCostData || [];
  const complianceStatusData = data?.complianceStatusData || [];
  const fuelUsageData = data?.fuelUsageData || [];
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      ...
                      labelLine={false}
                    >
                      {studentDeptData.map((entry: any, index: number) => (
